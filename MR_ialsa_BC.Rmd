...
- `r nrow(athlos_studies %>% filter(acronym %in% study_network$acronym) %>% select(acronym))` ATHLOS studies not in IALSA (ALSA, ELSA, HRS, LASA, SHARE, TILDA)
- `r length(mindmap_studies$acronym)` MINDMAP studies
- `r nrow(mindmap_studies %>% filter(acronym %in% study_network$acronym) %>% select(acronym))` MINDMAP studies not in IALSA (CLSA, LASA)
- `r length(near_studies$acronym)` NEAR studies
- `r nrow(near_studies %>% filter(acronym %in% study_network$acronym) %>% select(acronym))` NEAR studies not in IALSA (Betula, GENDER, H-70, KP, OCTO-Twin, SATSA, SNAC-K, SWEOLD)
